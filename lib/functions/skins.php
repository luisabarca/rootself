...
function pi_skin_google_fonts(){
	$query = array();
	$google_query = 'http://fonts.googleapis.com/css?family=';
	$fonts = pi_get_font_faces();
	$opts_with_font = array('skin_body_font', 'skin_h1', 'skin_h2', 'skin_h3', 'skin_h4', 'skin_h5', 'skin_h6', 'skin_header_logo_font', 'skin_header_menu_font', 'skin_subheader_font', 'skin_footer_font');
	foreach( $opts_with_font as $opt_id ){
		$font = of_get_option( $opt_id );
		if( !in_array($fonts[$font['face']]['request'], $query) && $fonts[$font['face']]['type'] == 'google' ){
			$query[] = $fonts[$font['face']]['request'];
		}
	}
	if( !empty( $query ) ){
		$google_query .= implode( '|', $query );
		echo "<!-- Google Fonts -->\n";
