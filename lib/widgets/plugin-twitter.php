	...
	if ($username == '') {
		...
		if($update) {				
		  ...
		  if ( ( abs( time() - $time) ) < 86400 )
			$h_time = sprintf( __('%s ago', 'theme_textdomain'), human_time_diff( $time ) );
		  else
			$h_time = date(__('Y/m/d', 'theme_textdomain'), $time);

		  echo sprintf( __('%s', 'theme_textdomain'),' <em class="twitter-timestamp">' . $h_time . '</em>' );
		 }          
