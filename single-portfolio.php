						...
						<div id="portfolio-meta" class="clearfix">
							...
							<?php if( $imgs['image-1'] !='' || $imgs['image-2'] !='' || $imgs['image-3'] !='' || $imgs['image-4'] !='' || $imgs['image-5'] !='' || $imgs['image-6'] !='' ){ ?>	
								<div id="slider" class="nivo-slider">
									<?php for($i=1; $i<=6; $i++) : ?>
										<?php if($imgs['image-'.$i] !='') : ?>
											<?php $image = vt_resize( '', $imgs['image-'.$i] , 940, of_get_option('portfolio_nivo_height'), true ); ?>
							...
							<?php }elseif( $video['vimeo-youtube'] != '' || $video['embedded-code'] != '' ){ ?>
								...
								<div class="post-video">
									<?php pi_embed_video(get_the_ID(), '', 940, of_get_option('portfolio_single_height'), 'portfolio_video'); ?>
								</div>
							
							<?php }elseif( has_post_thumbnail() ){ ?>
								<div class="post-thumb"><?php pi_resize_img("width=940&height=" . of_get_option('portfolio_single_height') ); ?></div>
							<?php } ?>
						<!-- END .portfolio-meta -->	
						</div>
